...
class GrannyBase(object):
    """
    Base class for Granny - a Computer Vision software to perform fruit assessment from image.
    Granny's subclasses includes:
        - GRANNY_Segmentation: implementation of Mask-RCNN - an instance segmentation machine learning model, to extract instances from image input.
        - GRANNY_SuperficialScald: image processing to threshold superficial scald in "Granny Smith" apples.
        - GRANNY_PeelColor: image processing to extract green-yellow mean values on apple's/pear's peel.
        - and more ...
    """

    def __init__(
        self, action: str = "", fname: str = "", num_instances: int = 1, verbose: int = 0
    ):
        # current directory
    ...
    def list_all(self, data_dir: str = os.path.curdir) -> Tuple[List[str], List[str]]:
        """
        Recursively list all the folder names and image file names in the directory
        """
        file_name: List[str] = []
